...
"自动定位"依赖预编译的 C# 工具(UIA + Win32 远程内存)枚举资源管理器/桌面的文件列表,
按文件名匹配后返回图标坐标。已做大量兼容(per-monitor DPI 感知、UIA 坐标空间自校准、
UTF-8/UTF-16/GBK 三编码解码、三通道检索 + 全局 UIA 兜底),但以下场景仍可能出现
**定位不准或定位失败**:

| 场景 | 现象 | 原因 |
|---|---|---|
| 不同分辨率 / 不同缩放倍数(DPI) | 怪兽指向位置偏离文件图标 | 坐标换算依赖屏幕缩放,个别显示器/显卡驱动下 UIA 返回的坐标空间与物理像素不一致,启发式校准仍有误差 |
| 不同系统默认编码(UTF-8 / UTF-16 / GBK) | 找不到文件 → 定位失败,退回手动瞄准 | 桌面视图的列表文本按系统 ANSI 代码页写入(中文系统=GBK),已尝试三种解码,个别改过区域设置/语言包的系统仍会乱码导致匹配不上 |
| 安装桌面整理软件(腾讯桌面整理等) | 桌面上的文件定位偏差或失败 | 这类软件接管了桌面图标,标准列表视图/UIA 树结构被改写,读到的坐标与真实图标位置对不上 |

**兜底机制**:定位失败不会瞎猜光标位置(光标停在右键菜单上,必然瞄歪),而是自动切换
手动瞄准模式——十字准星 + 鼠标点击目标,功能完全可用。

**其他已知/未知问题**不再继续修复。本项目定位已尽力兼容,但想做到所有电脑、所有环境
下自动定位都精确,投入产出比太低。有能力者欢迎 Fork 继续开发(诊断线索见下)。

**远程排查线索**:每次召唤都会向 `%APPDATA%\monster-deleter\find-file-rect.log` 追加一行
诊断日志(工具 DPI 感知模式、命中通道、坐标、退出码),反馈定位问题时附上该日志即可定位。
